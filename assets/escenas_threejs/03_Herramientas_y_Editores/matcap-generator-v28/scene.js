    ...
    <div class="panel-body">
      <div style="display: flex; flex-direction: column; gap: 6px;">
        ...
        <select class="panel-select" id="pDemoModels" style="width:100%;">
          ...
          <option value="MaterialsVariantsShoe">👟 Shoe</option>
...
const demoModels = {
  ...
  SciFiHelmet: {
    url: 'https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/main/2.0/SciFiHelmet/glTF-Binary/SciFiHelmet.glb',
    name: '🛡️ Sci-Fi Helmet',
    creator: 'Michael Pavlovich (Quixel)',
    source: 'Khronos glTF Sample Models',
    sourceUrl: 'https://github.com/KhronosGroup/glTF-Sample-Models/tree/main/2.0/SciFiHelmet',
    license: 'CC-BY 4.0',
    licenseUrl: 'https://creativecommons.org/licenses/by/4.0/',
  },
...
function tryLoadModel(urlIndex) {
  ...
  // Update loading display with the correct model name for fallback demo models
  const urlMeta = findModelMetaByURL(url);
  if (urlMeta) {
    const loadLabel = loadingDiv.querySelector('div');
    if (loadLabel) loadLabel.textContent = `Loading ${urlMeta.name.replace(/^.\s*/, '')}`;
  }
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      if (meta) {
        ...
        // Extract a clean name from the loaded URL or the uploaded filename
        const urlName = url.split('/').pop()?.split('?')[0] || '';
        const uploadName = modelData?.name || '';
        if (uploadName && !uploadName.includes('component.glb')) {
          displayModelName = uploadName.replace(/\.[^/.]+$/, '');
        } else if (urlName) {
          displayModelName = urlName.replace(/\.[^/.]+$/, '');
        } else {
          displayModelName = modelFileName;
        }
      }
