...
BUILD_ID_VALUE="${BUILD_ID:-local}"
BUILD_DIR="/tmp/build_fullstack_${BUILD_ID_VALUE}"
...
# PostgreSQL 在部署环境中独立提供，这里不再打包本地数据库文件
echo "🗄️  PostgreSQL 模式已启用：跳过 SQLite 文件打包"
echo "ℹ️  请在部署后执行 Prisma migrations（例如: npm run db:migrate:deploy）"
